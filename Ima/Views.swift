...
struct CreateTaskView: View {
    ...
    var body: some View {
        NavigationStack {
            VStack(alignment: .leading, spacing: 24) {
                ...
                // Duration selection with slider first
                    ...
                    VStack(spacing: 8) {
                        HStack {
                            Slider(
                                ...
                                in: 1800...settings.maxAllowedDuration,
                                step: 1800
                            ) {
                                Text("Duration")
                            }
                            
                            Text(formatRemaining(selectedDuration))
                        ...
                        HStack(spacing: 8) {
                            ForEach(presetDurations, id: \.1) { label, duration in
                                ...
                                .liquidGlassButton(prominent: selectedDuration == duration)
                            }
                        }
                    }
                }
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    TextField("Link (URL)", text: $linkText)
                        .textFieldStyle(.roundedBorder)
                    
                    if !linkText.isEmpty {
                        HStack(spacing: 4) {
                            if URL.isValidURLFormat(linkText) {
    ...
    private func createTask() {
        ...
        let cappedDuration = min(selectedDuration, settings.maxAllowedDuration)
        
        let url = URL.validURL(from: linkText)
        let loc = location.isEmpty ? nil : location
        
        store.add(title: trimmedTitle, duration: cappedDuration, location: loc, link: url)
        isPresented = false
    }
}
